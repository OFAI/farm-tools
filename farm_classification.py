"""
Library and program for FARM-based BERT document classification.
IMPORTANT: the code in here is mostly copypasta for now and this is planned for later!
For now we use the bert_class* programs to perform specific tasks.
Eventually we may want to refactor code from there to create this lib and only use
lib api calls from those other scripts eventually!
"""
import logging
import json
...
default_xval_cfg = {
    "mlflow_logger": "logs",
    # "mlflow_logger": "https://public-mlflow.deepset.ai/", # Or None
    "exp_name": "FemdwellTest",
    "xval_folds": 5,
...
def doc_classification_crossvalidation(vocab_file=None, cfg=None):
    if cfg is None:
        cfg = default_xval_cfg
    ...
    ml_logger = None
    if cfg.get("mlflow_logger"):
        ml_logger = cfg.get("mlflow_logger")
        ml_logger.init_experiment(
            experiment_name=cfg.get("exp_name"),
            run_name=cfg.get("exp_run", "TextXval"))

    ##########################
    ########## Settings
    ##########################
    xval_folds = cfg.get("xval_folds")
    xval_stratified = cfg.get("xval_stratified")

    set_all_seeds(seed=cfg.get("random_seed"))
    device, n_gpu = initialize_device_settings(use_cuda=cfg.get("use_cuda"))
    n_epochs = cfg.get("max_epochs")
    batch_size = cfg.get("batch_size")
    evaluate_every = cfg.get("eval_every")
    lang_model = cfg.get("lang_model")
    use_amp = None  # TODO: ?????
